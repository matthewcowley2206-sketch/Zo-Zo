...
export function ServiceThemeCard({ theme, className = '' }: ServiceThemeCardProps) {
  const isFeatured = theme.featured
    ...
    >
      <p
        className={`text-[0.6875rem] font-semibold uppercase tracking-[0.14em] ${
          isFeatured ? 'text-cream/55' : 'text-muted-light'
        }`}
      >
      ...
      <p
        className={`mt-3 flex-1 text-[0.9375rem] leading-relaxed ${
          isFeatured ? 'text-cream/75' : 'text-muted group-hover:text-ink/80'
        }`}
      >
        {theme.summary}
      </p>
      <ul className={`mt-5 space-y-2 ${isFeatured ? 'text-cream/70' : 'text-muted'}`}>
        {theme.modules.map((module) => {
          ...
          return (
            <li
              ...
              className={
                isTbyi
                  ? `text-[0.8125rem] ${tbyiCapabilityLinkClasses} rounded-2xl border px-3 py-2.5`
                  : 'text-[0.8125rem]'
              }
            >
              {isTbyi ? (
                <TbyiCapabilityHeader
                  title={module.label}
                  titleClassName={`${tbyiCapabilityTitleClasses} text-[0.9375rem]`}
                />
              ) : (
              ...
              {module.supportingLine ? (
                <span
                  className={`mt-0.5 block pl-3 text-[0.75rem] leading-relaxed ${
                    isTbyi
                      ? `${tbyiCapabilityBodyClasses} pl-0`
                      : isFeatured
                        ? 'text-cream/55'
                        : 'text-muted-light'
                  }`}
